...
import 'package:hoho_hanja/screens/cookie/cookie_screen.dart';
import 'package:hoho_hanja/utils/network_check.dart';
import 'package:hoho_hanja/widgets/dialog/dialog.dart';
import 'package:logger/logger.dart';

// 쿠키팡
Future<void> cookieService(String phase, code) async {
  final connectivityController = Get.put(ConnectivityController());

  if (connectivityController.isConnected.value) {
    String url = dotenv.get('COOKIE_URL');
      ...
      if (response.statusCode == 200) {
        ...
        if (resultValue == "0000") {
          List<CookieData> cookieDataList =
              resultList.map((item) => CookieData.fromJson(item)).toList();
          final CookieDataController cookieDataController =
              Get.put(CookieDataController());
          cookieDataController.setCookieDataList(cookieDataList);
          cookieDataController.quizList();
          Get.to(() => CookieScreen(code));
        }
